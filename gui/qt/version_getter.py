...
from electrum_XSH import ELECTRUM_VERSION, print_error
...
class VersionGetter(threading.Thread):
    ...
    def run(self):
        try:
            res = requests.request("GET", "http://electrum-SHIELD.xyz/version")
        except:
...
class UpdateLabel(QLabel):
    ...
    def open_website(self):
        webbrowser.open("http://electrum-SHIELD.xyz/download.html")
        self.dialog.done(0)

    def mouseReleaseEvent(self, event):
        dialog = QDialog(self)
        dialog.setWindowTitle(_('Electrum-XSH update'))
        dialog.setModal(1)

        main_layout = QGridLayout()
        main_layout.addWidget(QLabel(_("A new version of Electrum-XSH is available:")+" " + self.latest_version), 0,0,1,3)
