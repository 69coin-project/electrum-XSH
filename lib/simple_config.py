...
SYSTEM_CONFIG_PATH = "/etc/electrum-XSH.conf"
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-XSH.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-XSH.conf. please install ConfigParser"
            return
