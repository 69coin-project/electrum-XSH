...
class Commands:
    ...
    def createrawtx(self, inputs, outputs, unsigned=False):
        """Create a transaction from json inputs. The syntax is similar to SHIELDd."""
    ...
    def version(self):
        ...
        import electrum_XSH as electrum  # Needs to stay here to prevent ciruclar imports
...
command_options = {
    'broadcast':   (None, "--broadcast",   "Broadcast the transaction to the SHIELD network"),
    ...
    'tx_fee':      ("-f", "--fee",         "Transaction fee (in XSH)"),
...
config_variables = {
    ...
    'addrequest': {
        ...
        'url_rewrite': 'Parameters passed to str.replace(), in order to create the r= part of SHIELD: URIs. Example: \"(\'file:///var/www/\',\'http://electrum-SHIELD.xyz/\')\"',
    },
    'listrequests':{
        'url_rewrite': 'Parameters passed to str.replace(), in order to create the r= part of SHIELD: URIs. Example: \"(\'file:///var/www/\',\'http://electrum-SHIELD.xyz/\')\"',
    }
...
def get_parser(run_gui, run_daemon, run_cmdline):
    ...
    # gui
    parser_gui = subparsers.add_parser('gui', parents=[parent_parser], description="Run Electrum-XSH's Graphical User Interface.", help="Run GUI (default)")
